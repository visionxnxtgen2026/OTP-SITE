...
/**
 * Detect the user's country using browser locale and timezone (sync fallback)
 */
export const detectUserCountry = () => {
  ...
  return countriesList.find((c) => c.iso === 'IN') || countriesList[0];
};

/**
 * Detect the user's country using the priority sequence:
 * 1. Geolocation API
 * 2. Browser Locale
 * 3. User Timezone
 * 4. IP-based country detection from backend/API
 */
export const detectUserCountryAsync = async () => {
  const CACHE_KEY = 'dds_user_detected_country';
  ...
  try {
    ...
    if (cached) {
      ...
      const match = countriesList.find((c) => c.iso === parsed.iso);
      if (match) return match;
    }
  } catch (e) {
  ...
  const saveToCache = (countryObj) => {
    try {
      localStorage.setItem(CACHE_KEY, JSON.stringify(countryObj));
    } catch (e) {
      console.warn('[Cache Write Error]', e);
    }
  };

  const defaultFallback = countriesList.find((c) => c.iso === 'IN') || countriesList[0];

  const matchCountryByIso = (iso) => {
    if (!iso) return null;
    return countriesList.find((c) => c.iso === iso.toUpperCase());
  };

  // 1. Geolocation API (with user permission)
  if (navigator.geolocation) {
    try {
      ...
      const geoResponse = await fetch(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${latitude}&longitude=${longitude}`);
      if (geoResponse.ok) {
        const geoData = await geoResponse.json();
        const matched = matchCountryByIso(geoData.countryCode);
        if (matched) {
          saveToCache(matched);
          return matched;
        }
      ...
      console.warn('[Geolocation Detection Failed/Denied]', err.message);
    }
  }
  ...
  try {
    ...
    if (locale) {
      ...
      const matched = matchCountryByIso(region);
      if (matched) {
        saveToCache(matched);
        return matched;
      }
    ...
    console.warn('[Locale Detection Failed]', err.message);
  }
  ...
  try {
    const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    if (timezone && timezoneMap[timezone]) {
      const matched = matchCountryByIso(timezoneMap[timezone]);
      if (matched) {
        saveToCache(matched);
        return matched;
      }
  ...
  try {
    ...
    if (response.ok) {
      ...
      const matched = matchCountryByIso(data.countryCode);
      if (matched) {
        saveToCache(matched);
        return matched;
      }
    ...
    console.warn('[Backend IP Detection Failed]', err.message);
  }
