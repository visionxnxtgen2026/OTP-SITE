...
// Imports from the newly created worldwide index
    ...
    <div className="relative flex select-none" ref={dropdownRef}>
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          {!minimal && (
            <span className="text-xs font-extrabold tracking-tight text-text-primary">
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="p-2 border-b border-gray-100 flex items-center gap-2 bg-gray-50/50">
              <Search size={13} className="text-gray-450 ml-2" />
              <input
                type="text"
                placeholder="Search country name or code..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full bg-transparent border-0 outline-none text-xs text-text-primary placeholder-gray-400 py-1.5 focus:ring-0"
              />
            </div>
              ...
              {filteredCountries.length > 0 ? (
                filteredCountries.map((c) => (
                  ...
                  >
                    ...
                    <span className="text-gray-400 font-extrabold text-[10px] tracking-wide uppercase">{c.iso} ({c.code})</span>
                  </button>
                ))
              ) : (
                <div className="px-4 py-6 text-center text-xs text-gray-400 font-semibold select-none">
